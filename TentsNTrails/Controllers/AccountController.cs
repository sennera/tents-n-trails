...
namespace TentsNTrails.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            if (returnUrl != null && returnUrl.Equals("/Review/CreateShort"))
            {
                returnUrl = "/Location/Index";
            }
            else if (returnUrl != null && returnUrl.Contains("/Profile/Connect?"))
            {
                returnUrl = returnUrl.Replace("Profile/Connect", "Profile");
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new User { UserName = model.UserName, FirstName = model.FirstName, 
                    LastName = model.LastName, Email = model.Email, EnrollmentDate = DateTime.Now, Private = model.Private };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);

                    User currentUser = UserManager.FindByEmail(user.Email);//attempt to fix issue below
                    ...
                    foreach (var userRec in model.RecOptions)
                    {
                        if (userRec.IsChecked)
                        {
                            ...
                            ur.User = currentUser.Id;//fixed the issue. Now just go change the profile page to grab the relevent information about userRecs
